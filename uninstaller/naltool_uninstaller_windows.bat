...
:: NalTool Uninstaller for Windows
:: Made by Kacefier
...
echo NalTool Uninstaller v%VERSION%
echo Made by Kacefier
echo.
...
if exist "%INSTALL_DIR%\%BINARY_NAME%" (
    ...
    echo [INFO] Found NalTool at: !BINARY_PATH!
)

:: Check if it's in PATH
if "%FOUND%"=="false" (
    where naltool 2>nul > temp_path.txt
...
if "%FOUND%"=="false" (
    echo [WARN] NalTool not found in default location or PATH
    echo.
    set /p "CUSTOM_PATH=Enter the full path to naltool.exe (or press Enter to skip): "
...
if "%FOUND%"=="false" (
    echo [INFO] NalTool is not installed
    pause
    exit /b 0
)

...
print_header() {
    echo -e "${BLUE}NalTool Uninstaller v${VERSION}${NC}"
    echo -e "${BLUE}Made by Kacefier${NC}"
    echo ""
}
...
if [ -f "$INSTALL_DIR/$BINARY_NAME" ]; then
    ...
    FOUND=true
    print_info "Found NalTool at: $BINARY_PATH"
fi

# Check if it's in PATH
if [ "$FOUND" = false ]; then
    WHICH_PATH=$(which naltool 2>/dev/null || echo "")
...
if [ "$FOUND" = false ]; then
    print_warn "NalTool not found in default location ($INSTALL_DIR) or PATH"
    echo ""
    read -p "Enter the full path to the naltool binary (or press Enter to skip): " CUSTOM_PATH
...
if [ "$FOUND" = false ]; then
    print_info "NalTool is not installed"
    exit 0
fi
...
echo ""
read -p "Do you also want to remove NalTool entries from your shell profile files? (y/N): " REMOVE_PROFILE
...
if [[ "$REMOVE_PROFILE" =~ ^[Yy]$ ]]; then
    ...
    for FILE in "${PROFILE_FILES[@]}"; do
        if [ -f "$FILE" ]; then
            if grep -q "NalTool\|naltool" "$FILE" 2>/dev/null; then
                cp "$FILE" "$FILE.bak"
                print_info "Backup created: $FILE.bak"

                sed -i.bak '/# NalTool/d' "$FILE"
                sed -i.bak '/naltool/d' "$FILE"
                sed -i.bak '/NALTOOL/d' "$FILE"

                print_info "Removed NalTool entries from $FILE"
            fi
        fi
    done
...
echo ""
echo "NalTool has been removed from your system"
